...
interface CarListing {
  ...
  year: number;
  mileage: number; // km
  isWreck: boolean; // Ground truth (jestli je to vrak)
  isArbitrage: boolean; // Ground truth (jestli je to skutečně super deal)
}

// Generování mock dat pro "Porsche 911"
const mockMarket: CarListing[] = [
...
type Algorithm = (market: CarListing[], target: CarListing) => boolean;
...
const algorithms: Record<string, Algorithm> = {
  ...
  "V3_GridSegmentation": (market, target) => {
    const isSimilarMileage = (m1: number, m2: number) => Math.abs(m1 - m2) <= 15000;
  ...
  "V4_IQR_Filtering": (market, target) => {
    ...
    return target.price < avg * 0.85; // 15% pod očištěným průměrem
  },
  ...
  "V5_ZScore": (market, target) => {
    ...
    // Arbitráž = 1.5 až 3 sigma pod průměrem. Více než 3 sigma pod = vrak.
  ...
  "V6_MileageDecayFormula": (market, target) => {
    ...
    // Nalezení "nového" auta v daném ročníku a odvození degradace ceny za km
    const avgPrice = similar.reduce((sum, c) => sum + c.price, 0) / similar.length;
    const expectedPrice = avgPrice * (1 - (target.mileage / 500000)); // Hrubá aproximace
    
    return target.price < expectedPrice * 0.85 && target.price > expectedPrice * 0.50; // Nesmí to být <50% (vrak)
  },

  // 7. K-Neareast Neighbors (KNN)
  "V7_KNN_Pricing": (market, target) => {
    // Spočítá vzdálenost každého auta od targetu (rok váha 20000, km váha 1)
    const getDist = (c: CarListing) => Math.abs(c.year - target.year) * 200000 + Math.abs(c.mileage - target.mileage);
    
    const sorted = [...market].filter(c => c.id !== target.id).sort((a, b) => getDist(a) - getDist(b));
    const neighbors = sorted.slice(0, 3); // Top 3 nejbližší
    
    if (neighbors.length < 2) return false;
    const avg = neighbors.reduce((s, c) => s + c.price, 0) / neighbors.length;
    
    return target.price < avg * 0.85 && target.price > avg * 0.5; // opět ochrana proti vraku
  },
  ...
  "V8_HybridGridZScore": (market, target) => {
    ...
    // Musí to být aspoň 12% pod průměrem, ale nesmí to být nesmyslně levné (zScore < -2.5)
  ...
  "V10_AntigravityMatrix": (market, target) => {
    // 1. Najdi jen auta v okruhu +-1 rok
    const cohort = market.filter(c => Math.abs(c.year - target.year) <= 1);
    if (cohort.length < 3) return false;
    
    // 2. Normalizuj cenu přes km korelaci (velmi jednoduchá lineární regrese)
    // TBD: Pro POC použijeme median a IQR filter pro vyhození vraků
    const prices = cohort.map(c => c.price).sort((a, b) => a - b);
    const q1 = prices[Math.floor(prices.length * 0.25)];
    const q3 = prices[Math.floor(prices.length * 0.75)];
    const iqr = q3 - q1;
    const cleanMarket = cohort.filter(c => c.price >= q1 - 1.5 * iqr); // vyhodí dolní extrémy (vraky)

    if (cleanMarket.length < 2) return false;

    // 3. Spočítá dynamickou tržní cenu na základě nájezdu v clean marketu
    // Nejbližší sousedé z očištěného trhu
    const getDist = (c: CarListing) => Math.abs(c.mileage - target.mileage);
    const peers = cleanMarket.sort((a, b) => getDist(a) - getDist(b)).slice(0, 3);
    const peerAvg = peers.reduce((s, c) => s + c.price, 0) / peers.length;

    // 4. Musí být -15% pod přesným peer průměrem, ale nesmí to padnout víc než -40% pod něj (pak je to past)
    return target.price <= peerAvg * 0.85 && target.price >= peerAvg * 0.60;
  }
};
...
Object.entries(algorithms).forEach(([name, algo]) => {
  let truePositives = 0; // Našel skutečnou arbitráž
  let falsePositives = 0; // Označil vrak nebo normální auto jako arbitráž
  let falseNegatives = 0; // Přehlédl skutečnou arbitráž

  mockMarket.forEach(car => {
    // Modelujeme situaci, kdy algoritmus zná celý trh kromě tohoto jednoho auta, které právě přišlo
