...
import { createDb } from '../../spine/engine/acquisition/esbirka/db.js';
import { runDiscovery } from '../../spine/engine/acquisition/esbirka/discovery.js';
import { runDetail } from '../../spine/engine/acquisition/esbirka/scraper.js';
import type { ScraperConfig } from '../../spine/engine/acquisition/esbirka/types.js';
...
console.log('eSbírka Scraper (Ministerstvo vnitra ČR) [Running via Antigravity]');
...
const run = async (overridePhase?: string) => {
  const currentPhase = overridePhase || config.phase;
  try {
    if (currentPhase === 'all' || currentPhase === 'sitemap' || currentPhase === 'discovery') {
      await runDiscovery(db, config, shutdown.isShuttingDown);
    ...
    if (currentPhase === 'all' || currentPhase === 'detail') {
      ...
      await runDetail(db, config, shutdown.isShuttingDown);
    }
  } catch (error) {
...
if (values.daemon) {
  ...
  const listener = createCommandListener('esbirka', async (payload) => {
