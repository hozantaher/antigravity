        ...
        result: {
          tools: [
            ...
            },
            {
              ...
              description:
                'Checks the vector-tree architecture for broken links and heals them if requested.',
              inputSchema: {
                type: 'object',
            ...
            },
            {
              ...
              description:
                'Generates a new vector-tree node, inferring context and creating boilerplate files and reverse links.',
              inputSchema: {
                type: 'object',
                properties: {
                  nodeId: { type: 'string' },
            ...
            },
            {
              ...
              description:
                'Searches the vector tree using fuzzy matching on node intent, tags, and structure.',
              inputSchema: {
                type: 'object',
                properties: {
                  query: { type: 'string' },
            ...
            },
            {
              ...
              description:
                'Generates a global architecture map with a Mermaid graph of the entire vector tree.',
              inputSchema: {
                type: 'object',
                properties: {},
              },
      ...
      try {
        ...
        if (toolName === 'antigravity_audit_drift') {
          ...
          if (args.heal && report.length > 0) {
            this.diary.logAction(
              'AI',
              'AUDIT_HEAL',
              `Proveden audit s auto-heal. Nalezeno a opraveno ${report.length} problémů.`
            );
          }
          return {
            jsonrpc: '2.0',
        ...
        if (toolName === 'antigravity_rename_node') {
          ...
          this.diary.logAction(
            'AI',
            'RENAME_NODE',
            `Přejmenován uzel z ${args.oldId} na ${args.newId}`,
            [args.newId]
          );
          ...
          return {
            ...
            result: {
              content: [
                {
                  type: 'text',
                  text: `Node renamed successfully. Plan executed:\n${plan.join('\n')}`,
                },
              ],
            },
          };
        ...
        if (toolName === 'antigravity_create_node') {
          ...
          this.diary.logAction(
            'AI',
            'CREATE_NODE',
            `Vytvořen uzel ${args.nodeId} v cestě ${args.pathHint}`,
            [args.nodeId]
          );
