...
func TestGenerateV3OnionKeyFailsOnReadOnlyDir(t *testing.T) {
	// A chmod 0500 dir is still writable by root (the CI runner runs as root),
	// so force the write failure in a uid-independent way: point at a path whose
	// parent is a regular file → ENOTDIR for any uid.
	base := t.TempDir()
	dir := filepath.Join(base, "not-a-dir")
