...
func TestAntiTraceClient_Send_IncludesSMTPCreds(t *testing.T) {
	...
	// No request-level SMTPUsername: this test asserts the CLIENT-level creds
	// (c.smtpUsername etc.) propagate to the relay payload. A request override
	// would win per Send's resolution order and mask what we're verifying.
	c.Send(context.Background(), SendRequest{ToAddress: "to@example.com", Subject: "Hi", BodyPlain: "body"})
