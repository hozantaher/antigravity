...
func TestAntiTrace_EnvelopeIDReturned(t *testing.T) {
	...
	for _, env := range cases {
		t.Run(fmt.Sprintf("env=%q", env), func(t *testing.T) {
			...
			res := NewAntiTraceClient(srv.URL, "t").
				Send(context.Background(), SendRequest{ToAddress: "to@x.cz", SMTPUsername: "sender@x.cz", SMTPPassword: "pw"})
...
func TestAntiTrace_MailboxUsedIsFromAddr(t *testing.T) {
	...
	defer srv.Close()
	res := NewAntiTraceClient(srv.URL, "t").
		Send(context.Background(), SendRequest{ToAddress: "to@x.cz", SMTPUsername: "mymailbox@firma.cz", SMTPPassword: "pw"})
...
func TestAntiTrace_ConcurrentSendSafe(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		go func() {
			cli.Send(context.Background(), SendRequest{ToAddress: "to@x.cz", SMTPUsername: "sender@x.cz", SMTPPassword: "pw"})
			done <- struct{}{}
		}()
	}
	for i := 0; i < 50; i++ {
