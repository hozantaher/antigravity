...
// TestAntiTraceClient_Send_EmptyCredentials verifies that a SendRequest with
// no SMTP credentials is rejected by the guard in AntiTraceClient.Send before
// any HTTP request reaches the relay. Engine rotation must inject per-mailbox
// creds (engine.go copies mailbox.Username/Password onto the request); an empty
// SMTPUsername is a misconfiguration, not a deliverable send, so the relay must
// never be asked to authenticate with a zero-value mailbox.
func TestAntiTraceClient_Send_EmptyCredentials(t *testing.T) {
	...
	if result.Error == nil {
		t.Fatal("expected missing-SMTPUsername guard error, got nil")
	}
	if !contains(result.Error.Error(), "missing SMTPUsername") {
		t.Errorf("error should report missing SMTPUsername, got %v", result.Error)
	}
	// The guard returns before issuing the HTTP request, so the relay never
	// sees a payload — gotPayload stays zero-valued.
