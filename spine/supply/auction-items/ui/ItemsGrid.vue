...
const { getCardImage, dprSrcset, fallbackUrl } = useImageProcessing()
...
const media = computed(() => {
  const m = new Map<string, { image: string; srcset: string; fallback: string }>()
  for (const item of props.items) {
    m.set(item.id, {
      image: getCardImage(item.image),
      srcset: dprSrcset(item.image, '380x280'),
      fallback: fallbackUrl(item.image, { width: 380, height: 280 }),
    })
  }
  return m
})
