...
const { t } = useI18n()
const { imgUrl, getImageUrl, widthSrcset, fallbackUrl } = useImageProcessing()
...
const currentFallback = computed(() => fallbackUrl(props.images[props.index] ?? '', { width: 1280 }))
...
watch(
  ...
  src => {
    ...
    // Probe what's displayed; if src errors (e.g. a missing derivative) probe the /img fallback that
    // BaseImage swaps to, so the spinner stays up until the actually-shown image settles — not cleared
    // prematurely on the derivative's error while the fallback is still loading.
    const probe = (url: string, onError: () => void) => {
      const img = new Image()
      img.onload = settle
      img.onerror = onError
      img.src = url
    ...
    probe(src, () => probe(currentFallback.value, settle))
  },
  { immediate: true },
)
