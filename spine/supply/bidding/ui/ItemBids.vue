...
const { bids, total, page, pageSize, refresh } = useItemBids(() => item.value?.bids)
const pageOffset = computed(() => (page.value - 1) * pageSize)

// A new bid prepends to the (now newest-first) history — jump back to page 1 to show it.
watch(() => item.value?.bids.length, refresh)
</script>
