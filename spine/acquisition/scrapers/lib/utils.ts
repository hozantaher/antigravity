...
export const createRateLimiter = (baseDelayMs: number) => {
  ...
  const onSuccess = () => {
    ...
    if (consecutiveSuccesses >= 20 && currentDelay > minDelay) {
      ...
      console.log(`  [rate-limiter] Delay decreased to ${currentDelay}ms`);
    }
  };
  ...
  const onRateLimited = (retryAfterSec?: number) => {
    ...
    console.log(`  [rate-limiter] Rate limited, delay -> ${currentDelay}ms, pausing ${pauseSec}s`);
  };
...
//
// M-S1 note (2026-04-22): `attempt <= maxRetries` is intentional.
// `maxRetries` counts RE-tries (additional attempts after the first), so
// total calls = 1 (initial) + maxRetries. Examples:
//   maxRetries=0 → 1 total call  (no retries)
//   maxRetries=3 → 4 total calls (1 initial + 3 retries)
// Callers relying on exact attempt counts should pass maxRetries = (desired
// total calls − 1). Tests in utils.test.ts pin this contract.
