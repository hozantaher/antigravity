...
describe('health server', () => {
  ...
  it('should start server on specified port', async () => {
    server = startHealthServer(testPort, 'test-scrapers');
  ...
  it('should respond to GET /healthz with 200 ok', async () => {
    ...
    expect(data.service).toBe('test-scrapers');
  ...
  it('should include service name in response', async () => {
    const serviceName = 'my-custom-scrapers';
  ...
  it('should have uptime starting from zero', async () => {
    ...
    resetStartTime();
    server = startHealthServer(uniquePort, 'test-scrapers');
    await waitForListening(server);
