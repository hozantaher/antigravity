...
// Persists the billing address so the Fakturoid proforma carries it.
const onNext = async () => {
  if (!isFormValid([fieldStreet, fieldCity, fieldZip, fieldCountry])) return
  // The fire-and-forget fetch above may still be in flight on a fast submit —
  // without this the Continue button silently does nothing.
  if (!countries.value.length) await fetchCountries()
  const country = findCountryByCode2(countryCode.value)
  if (!country) {
    toast.error(t('deposit.billing.error'))
    return
  }
...
<template>
  <div class="step">
    <h4 class="step-heading">{{ t('deposit.billing.heading') }}</h4>
    <p class="step-intro">{{ t('deposit.billing.intro') }}</p>

    <div class="identity">
      <div class="identity-row">
        <span class="identity-label">{{ t('deposit.billing.name') }}</span>
        <span class="identity-value">{{ user?.fullName }}</span>
      </div>
      <div class="identity-row is-last">
        <span class="identity-label">{{ t('deposit.billing.email') }}</span>
        <span class="identity-value">{{ user?.email }}</span>
      </div>
    </div>

    <div class="form-fields">
      <BaseInput ref="fieldStreet" v-model:value="street" type="text" :label="t('deposit.billing.street')" required />
      <div class="form-pair">
        <BaseInput ref="fieldZip" v-model:value="zip" type="text" :label="t('deposit.billing.zip')" required />
        <BaseInput ref="fieldCity" v-model:value="city" type="text" :label="t('deposit.billing.city')" required />
      </div>
      <BaseSelect
        ref="fieldCountry"
        v-model:value="countryCode"
        :label="t('deposit.billing.country')"
