...
// MailboxDebugState is a point-in-time snapshot of one mailbox's in-memory
// send-eligibility inputs. Diagnostic only (incident 2026-06-27 —
// subset-of-mailboxes-sending).
type MailboxDebugState struct {
	Address         string
	SentCount       int
	EffectiveLimit  int
	DailyLimitCfg   int
...
// LogDebugSnapshot emits per-mailbox in-memory send-eligibility state so an
// operator can see WHY pickMailbox skips a mailbox (sent_count vs the exact
// effective_limit it derives, cooldown, registry allow-set membership) without
// per-pick log spam. Read-only; gate the caller behind an env flag.
//
// effective_limit mirrors pickMailbox's derivation (see the daily-limit block
// in pickMailbox) MINUS the warmupLimiter DB lookup, which is kept out of the
// engine mutex; when mailbox_warmup has no row it returns the same value, so
// the snapshot is exact for the empty-table case. Added for incident
// 2026-06-27, where the throttle was localized to in-memory engine state with
// no live debug surface.
func (e *Engine) LogDebugSnapshot() {
	e.mu.Lock()
	now := time.Now()
	allowSize := -1
	if e.registryAllowed != nil {
		allowSize = len(e.registryAllowed)
	}
	states := make([]MailboxDebugState, 0, len(e.mailboxes))
	for _, mb := range e.mailboxes {
		limit := mb.DailyLimit
		if mb.WarmupDay > 0 {
			limit = e.warmupLimit(mb.WarmupDay)
		...
		cd := 0
		if until, ok := e.mailboxCooldownUntil[mb.Address]; ok && now.Before(until) {
			cd = int(until.Sub(now).Seconds())
		}
		inAllow := true
		if e.registryAllowed != nil {
			_, inAllow = e.registryAllowed[mailbox.NormaliseAddress(mb.Address)]
		}
		states = append(states, MailboxDebugState{
			Address:         mb.Address,
			SentCount:       e.sentCounts[mb.Address],
			EffectiveLimit:  limit,
			DailyLimitCfg:   mb.DailyLimit,
			WarmupDay:       mb.WarmupDay,
			CooldownRemainS: cd,
			InAllowSet:      inAllow,
		})
	}
	idx := e.currentIdx
	lastReset := e.lastDailyReset
	mbCount := len(e.mailboxes)
	e.mu.Unlock()

	slog.Info("sender debug snapshot",
		"op", "engine.LogDebugSnapshot/header",
		"mailboxes_in_engine", mbCount,
		"current_idx", idx,
		"last_daily_reset", lastReset.Format(time.RFC3339),
		"allow_set_size", allowSize,
	)
	for _, s := range states {
		slog.Info("sender debug snapshot mailbox",
			"op", "engine.LogDebugSnapshot/mailbox",
			"mailbox", s.Address,
			"sent_count", s.SentCount,
			"effective_limit", s.EffectiveLimit,
			"daily_limit_cfg", s.DailyLimitCfg,
			"warmup_day", s.WarmupDay,
			"cooldown_remaining_s", s.CooldownRemainS,
			"in_allow_set", s.InAllowSet,
			"skipped_by_cap", s.SentCount >= s.EffectiveLimit,
		)
	}
}
