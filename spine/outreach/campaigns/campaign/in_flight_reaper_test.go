...
// ── Graceful-shutdown reclaim: threshold 0 reaps even a brand-new in_flight ──
//
// The SIGTERM shutdown hook (orchestrator/cmd/outreach/main.go) constructs
// NewInFlightReaperWithThreshold(db, 0) so a redeploy reclaims its active batch
// immediately instead of stranding it for IN_FLIGHT_STUCK_THRESHOLD_HOURS. This
// locks that semantic: at threshold 0 a fresh (age ~0) candidate IS reaped,
// whereas the 24h reaper (TestAW73_FreshContact1h_NotReaped) leaves it alone.
func TestShutdownReclaim_ZeroThreshold_ReapsFreshContact(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
	...
	fresh := time.Now() // age ~0 — untouched by the periodic 24h reaper
	mock.ExpectQuery(reaperSelectRE.String()).
		WillReturnRows(makeReaperRows([]stuckCandidate{
			{id: 101, campaignID: 1, contactID: 201, createdAt: fresh},
		}))
	mock.ExpectExec(reaperUpdateRE.String()).
		WithArgs(int64(101)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	...
	r := NewInFlightReaperWithThreshold(db, 0)
	reaped, err := r.Run(context.Background())
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
	if reaped != 1 {
		t.Errorf("reaped count = %d, want 1 (threshold 0 must reclaim fresh in_flight on shutdown)", reaped)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("expectations: %v", err)
	}
