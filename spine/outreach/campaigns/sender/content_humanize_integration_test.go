...
// Control test: same wiring, template without marker — hook DOES run.
func TestContentToSender_NoMarkerAllowsHumanize(t *testing.T) {
	body := `{{/* subject: Nabídka */}}
Dobrý den,
...
`
	...
	if rendered.SkipHumanize {
		t.Fatal("no-marker template should have SkipHumanize=false")
	}
	...
	if req.Subject == rendered.Subject {
		t.Error("no-marker template should allow hook to mutate subject")
	}
}
