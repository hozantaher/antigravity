...
const SITEMAP_INDEX_URL = 'https://autoline.cz/sitemap.xml';
...
export const runSitemapPhase = async (db: ScraperDb, onShuttingDown: () => boolean) => {
  ...
  const sitemapUrls = parseSitemapIndex(indexXml, (url) => url.includes('sitemap-adverts'));
