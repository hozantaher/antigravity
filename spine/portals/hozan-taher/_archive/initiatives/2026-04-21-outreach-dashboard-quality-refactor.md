...
- [x] `useResource` hook + testy — `apps/outreach-dashboard/src/hooks/useResource.js` + `useResource.test.jsx`
- [x] `dbMutate` / `dbMutateDetached` + ring buffer + testy — `apps/outreach-dashboard/src/lib/dbMutate.js` + `dbMutate.test.js`
- [x] Job tracker (`createJob`, `runJob`, `getJob`, `listJobs`) + testy — `apps/outreach-dashboard/src/lib/jobs.js` + `jobs.test.js`
- [x] `createMailboxAlert` dedup helper + testy — `apps/outreach-dashboard/src/lib/mailboxAlerts.js` + `mailboxAlerts.test.js`
- [x] `StaleIndicator` komponenta — `apps/outreach-dashboard/src/components/StaleIndicator.jsx`
- [x] `getRelayBase` + `relayFetch` + testy — `apps/outreach-dashboard/src/lib/relayClient.js` + `relayClient.test.js`
