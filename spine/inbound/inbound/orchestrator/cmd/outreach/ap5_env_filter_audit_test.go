...
// to Go production query paths in services/orchestrator/.
//
...
//   - services/orchestrator/cmd/anonymity-harvest/main.go (loadMailboxes)
//   - services/orchestrator/cmd/anonymity-test/main.go (loadMailboxes)
//   - services/orchestrator/intelligence/mailbox_score_loop.go (loadActiveMailboxes)
//   - services/orchestrator/intelligence/stubs.go (stubbed path — has filter)
...
func ap5RepoRoot(t *testing.T) string {
	...
	// file: services/orchestrator/cmd/outreach/ap5_env_filter_audit_test.go
	// Root is 4 levels up.
...
func TestAP5_EnvFilter_AnonymityHarvest(t *testing.T) {
	root := ap5RepoRoot(t)
	content := ap5ReadFile(t, root, "services/orchestrator/cmd/anonymity-harvest/main.go")
...
func TestAP5_EnvFilter_AnonymityTest(t *testing.T) {
	root := ap5RepoRoot(t)
	content := ap5ReadFile(t, root, "services/orchestrator/cmd/anonymity-test/main.go")
...
func TestAP5_EnvFilter_MailboxScoreLoop(t *testing.T) {
	root := ap5RepoRoot(t)
	content := ap5ReadFile(t, root, "services/orchestrator/intelligence/mailbox_score_loop.go")
...
var allowedAP5NonTestFiles = map[string]string{
	"services/orchestrator/cmd/anonymity-harvest/main.go": "lookupMailboxIDByAddress: single-row reverse lookup by address, not a set query",
}
...
func TestAP5_EnvFilter_WhitelistOnlyInTestOrUtil(t *testing.T) {
	...
	orchRoot := filepath.Join(root, "services", "orchestrator")
	violations := []string{}
