...
export default function useAdminItem() {
  ...
  const fetchItem = async (id?: string): Promise<Item> => {
    ...
    // Admin endpoint (requireAdmin): full bid history for the in-memory bid pager + hidden drafts.
    // The public /api/item/:id is slim (last bid only), which would empty the editor's bid tab.
    const data = await $fetch<Item>(`/api/admin/item/${id}`)
