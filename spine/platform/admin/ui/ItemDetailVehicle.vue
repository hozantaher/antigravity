...
const { decoding, canDecode, decode, result, showResult, overwrite, applyResult } = useAdminItemVinDecode(item)
  ...
  <section v-if="item" class="vehicle">
    ...
    <BaseModal v-model:is-open="showResult" :size="ModalSize.Large" heading="VIN decode result" is-closable>
      <div v-if="result" class="decoded">
        <div class="meta">
          <span class="badge" :class="result.cached ? 'badge-cache' : 'badge-fresh'">
            {{ result.cached ? 'From cache (free)' : 'Freshly decoded' }}
          </span>
          <span v-if="!result.cached && priceLabel" class="price">Cost: {{ priceLabel }}</span>
