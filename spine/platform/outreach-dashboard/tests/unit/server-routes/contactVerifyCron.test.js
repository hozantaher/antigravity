...
describe('T01 — feature flag disabled → early return', () => {
  ...
  it('returns without touching DB when flag is off', async () => {
    const { pool, runContactVerifyCron } = setupCron()
...
describe('T02 — daily budget exhausted', () => {
  ...
  it('returns early when used >= dailyMax (500 default)', async () => {
    ...
    await runContactVerifyCron()
    // only 1 query (budget check); no due-picker, no verifyEmail
...
describe('T03 — picks due contacts', () => {
  ...
  it('runs verifyEmail once per due contact row', async () => {
    const pool = poolWithSequence([
      ...
      { rows: [] },  // UPDATE verifying
      { rows: [] },  // UPDATE with result
      { rows: [] },  // INSERT log
    ])
...
describe('T04 — skip bounce_hold in SQL predicate', () => {
  ...
  it('due-picker SQL excludes bounce_hold and spamtrap', async () => {
    const pool = makePool({ query: vi.fn()
      .mockResolvedValueOnce({ rows: [{ used: 0 }] })   // budget
      .mockResolvedValueOnce({ rows: [] })                // no due (filtered by SQL)
    })
    const verifyEmail = makeVerifyEmail()
    const { runContactVerifyCron } = mountContactVerifyCron({ ...BASE_DEPS, pool, verifyEmail, domainProbeLock: new Map() })
    await runContactVerifyCron()
    // Check the due-picker query includes NOT IN filter
    const duePicker = pool.query.mock.calls[1][0]
...
describe('T08 — successful verify → updates 5 fields + INSERT log row', () => {
  ...
  it('UPDATE contacts with 6 params + INSERT email_verification_log', async () => {
    ...
    const pool = { query: vi.fn().mockImplementation((sql, params) => {
      callCount++
      sqlCalls.push([String(sql).trim().replace(/\s+/g, ' '), params])
      if (callCount === 1) return Promise.resolve({ rows: [{ used: 0 }] })
...
describe('T09 — failed verify (timeout) → increments attempts + retry-next_at', () => {
  ...
  it('on timeout: increments attempts and sets retry-at', async () => {
    ...
    // UPDATE with incremented attempts (2+1=3) + retry date
    ...
    expect(retryUpdate.params[0]).toBe(3)  // attempts 2+1
    expect(retryUpdate.params[1]).toBeInstanceOf(Date)  // nextAt is a Date
  })
})
...
describe('T11 — 3 timeouts/h on same domain → quarantine fires', () => {
  ...
  it('inserts/upserts quarantine row when count >= 3', async () => {
    ...
    const pool = { query: vi.fn().mockImplementation((sql, params) => {
      callCount++
      queries.push({ sql: String(sql), params })
      if (callCount === 1) return Promise.resolve({ rows: [{ used: 0 }] })         // budget
      if (callCount === 2) return Promise.resolve({ rows: [                         // due contacts
        { id: 9, email: 'a@spam.cz', email_domain: 'spam.cz', email_status: 'risky', email_verify_attempts: 1 },
      ]})
      if (callCount === 3) return Promise.resolve({ rows: [] })                      // UPDATE verifying
      if (callCount === 4) return Promise.resolve({ rows: [] })                      // retry UPDATE (error path)
      if (callCount === 5) return Promise.resolve({ rows: [] })                      // INSERT timeout log
      if (callCount === 6) return Promise.resolve({ rows: [{ n: 3 }] })             // quarantine count = 3
      return Promise.resolve({ rows: [] })                                           // quarantine INSERT
    })}
...
describe('T12 — inflight guard prevents concurrent runs', () => {
  ...
  it('second call returns immediately when first is in flight', async () => {
    let resolveFirst
    const pool = { query: vi.fn().mockImplementation(() => new Promise(r => { resolveFirst = r })) }
    const verifyEmail = makeVerifyEmail()
    const domainProbeLock = new Map()
    const { runContactVerifyCron } = mountContactVerifyCron({ ...BASE_DEPS, pool, verifyEmail, domainProbeLock })
    ...
    // Second call should return immediately (inflight guard)
    await runContactVerifyCron()
    // First call has only started (1 query call before stall)
    expect(pool.query.mock.calls.length).toBe(1)
    // Resolve first so the test cleans up
    resolveFirst({ rows: [{ used: 600 }] })
    await first
  })
})
