...
// W2-G (2026-04-29): per-fetch timeout. Pre-fix every fetch in this
// file was unbounded — a hung Railway MCP service blocked the worker
// indefinitely (BullMQ job grabs Redis lock, then awaits forever on
// initialize/listTools/callTool/oauth-approve). Worker concurrency
// degrades silently; idempotent retry never fires (job is "active"
// not "failed"). MAX_ITER_BUDGET_MS in worker/generate-odpor.ts is
// documented but not actually enforced inside the iteration loop.
//
// 30s default is generous for an OAuth round-trip and a single MCP
// tool call. Operators can tune via MCP_FETCH_TIMEOUT_MS env.
const fetchTimeoutMs = Number(process.env.MCP_FETCH_TIMEOUT_MS ?? 30_000);
